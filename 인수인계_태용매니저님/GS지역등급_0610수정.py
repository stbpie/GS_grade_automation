...
# 경로 수정
os.chdir(r"C:\Users\USER\Desktop\jw\GS지역등급산출등\인수인계_태용매니저님")

'''데이터 불러오기1'''
# 이 데이터들 갱신 시점이 언제쯤인지 알아내야한다.
gagu = pd.read_excel("data/거처의_종류별_거처__가구__가구원__시군구.xlsx",header=[0,1,2], dtype = str)
ingu = pd.read_excel("data/성__연령_및_세대구성별_인구__시군구.xlsx",header=[0,1], dtype = str)
soyu = pd.read_excel('data/거주지역_주택_소유물건수별_주택소유자수.xlsx', header = [0,1], dtype = str)
soduk = pd.read_excel('data/26.05 기준 연소득.xlsx', dtype = str)
jutaek= pd.read_excel('data/주택의_종류별_주택__읍면동_연도_끝자리_0__5___시군구_그_외_연도.xlsx', header = [0,1] ,dtype = str)
j_code = pd.read_excel("data/지역코드매핑.xlsx", dtype = str, names = ['지역코드1','jk_code'])

# 소득파일 전분기거 가져와서 지역코드 가져오고 형태 맞춰줌 - 다음 분기 작업때 바뀔수있음
soduk_past = pd.read_excel('data/GS지역등급 연소득 최신자료_2026년01월 기준.xlsx', dtype = str)
...
# 소득 파일에 지역코드 붙여줌
jiyuk = pd.read_excel('data/지역코드정리.xlsx', dtype = str)
...
aa = pd.read_excel("data/gs_지역등급_자동화관련.xlsx", sheet_name = '세부로직')
...
mapping_result = pd.read_excel('data/매핑.xlsx', dtype = str)
gs_grade = pd.read_csv("data/GS_ds_grade.csv", dtype = str)
